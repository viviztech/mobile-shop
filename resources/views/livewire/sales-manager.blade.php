...
new #[Layout('layouts.app')] class extends Component {
    ...
    public function save(): void
    {
        ...
        $data = [
            'cash_session_id' => $this->activeSessionId,
            'user_id' => Auth::id(),
    ...
    {
        $session = \App\Models\CashSession::where('user_id', Auth::id())
            ->where('status', 'open')
            ->first();
    ...
    {
        $session = \App\Models\CashSession::find($this->activeSessionId);
        if ($session) {
        ...
        <div class="max-w-4xl mx-auto px-4 py-4">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-6">
                    ...
                    @if($activeSessionId)
                        <button wire:click="$set('showEndSessionModal', true)"
                            class="px-4 py-2 bg-red-500/20 border border-red-500/30 text-red-400 hover:bg-red-500/30 font-semibold rounded-xl text-xs transition-all flex items-center gap-2">
...
<main class="max-w-4xl mx-auto px-4 py-6 pb-24">
    <!-- Success Toast -->
    @if($showSuccess)
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => { show = false; $wire.dismissSuccess() }, 3000)"
            x-transition class="fixed top-20 right-4 left-4 md:left-auto md:w-80 z-50">
            <div
                ...
                <div class="relative">
                    ...
                    @elseif($showSearchResults && strlen($searchQuery) >= 2)
                        <div
                            ...
                            No products found. <a href="{{ route('products') }}" class="text-violet-400 hover:underline">Add
                                to catalog?</a>
                        </div>
                    @endif
                </div>
    ...
    <div class="flex items-center justify-around">
        ...
        @if($showSessionModal)
            <div class="fixed inset-0 z-[100] flex items-center justify-center px-4">
                ...
                <div class="relative w-full max-w-md bg-slate-800 border border-white/10 rounded-3xl p-8 shadow-2xl">
                    <div class="w-16 h-16 rounded-2xl bg-violet-500/20 flex items-center justify-center mb-6 mx-auto">
                        <svg class="w-8 h-8 text-violet-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    <h3 class="text-2xl font-bold text-white text-center mb-2">Daily Session</h3>
                    <p class="text-slate-400 text-center mb-8">Please enter your opening cash balance to start the day.</p>

                    <form wire:submit.prevent="startSession" class="space-y-6">
                        <div>
                            <label class="block text-sm font-medium text-slate-300 mb-2">OPENING BALANCE (CASH IN HAND)
                                *</label>
                            <div class="relative">
                                <span
                                    class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-lg">₹</span>
                                <input wire:model="opening_balance" type="number" step="0.01" required
                                    class="w-full pl-10 pr-4 py-4 bg-white/5 border border-white/10 rounded-2xl text-white text-xl font-bold focus:outline-none focus:ring-2 focus:ring-violet-500 transition-all">
                            </div>
                        </div>
                        <button type="submit"
                            class="w-full py-4 bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 text-white font-bold rounded-2xl shadow-xl shadow-violet-500/20 transition-all">
                            START SESSION
                        </button>
                        <p class="text-[10px] text-slate-500 text-center uppercase tracking-widest">Cell Ulagam Management
                            System</p>
                    </form>
                </div>
            </div>
            ...
            <div class="fixed inset-0 z-[100] flex items-center justify-center px-4">
                ...
                <div class="relative w-full max-w-md bg-slate-800 border border-white/10 rounded-3xl p-8 shadow-2xl">
                    <div class="flex items-center justify-between mb-8">
                        <h3 class="text-xl font-bold text-white">Close Session</h3>
                        <button wire:click="$set('showEndSessionModal', false)" class="text-slate-400 hover:text-white">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @php
                        $session = \App\Models\CashSession::where('user_id', Auth::id())->where('status', 'open')->first();
                        $expected = $session ? $session->calculateExpectedBalance() : 0;
                    @endphp

                    <div class="space-y-6 mb-8">
                        <div class="flex justify-between p-4 bg-white/5 rounded-2xl text-sm">
                            <span class="text-slate-400">Opening Balance</span>
                            <span
                                class="text-white font-bold">₹{{ number_format($session->opening_balance ?? 0, 2) }}</span>
                        </div>
                        <div class="flex justify-between p-4 bg-white/5 rounded-2xl text-sm">
                            <span class="text-slate-400">Today's Sales</span>
                            <span class="text-emerald-400 font-bold">+ ₹{{ number_format($todayTotal, 2) }}</span>
                        </div>
                        <div class="flex justify-between p-4 bg-violet-500/10 border border-violet-500/20 rounded-2xl">
                            <span class="text-violet-300 font-medium">Expected Cash</span>
                            <span class="text-violet-400 font-bold text-xl">₹{{ number_format($expected, 2) }}</span>
                        </div>
                    </div>

                    <form wire:submit.prevent="endSession" class="space-y-6">
                        <div>
                            <label class="block text-sm font-medium text-slate-300 mb-2">ACTUAL CASH IN HAND *</label>
                            <div class="relative">
                                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold">₹</span>
                                <input wire:model="closing_balance" type="number" step="0.01" required
                                    class="w-full pl-10 pr-4 py-4 bg-white/5 border border-white/10 rounded-2xl text-white text-lg font-bold focus:outline-none focus:ring-2 focus:ring-violet-500 transition-all">
                            </div>
                        </div>
                        <button type="submit"
                            class="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-2xl shadow-xl transition-all">
                            END SESSION & VERIFY
                        </button>
                    </form>
                </div>
