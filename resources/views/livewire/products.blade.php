...
<div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        ...
        @else
            <div class="grid gap-4">
                @foreach($products as $product)
                    <div
                        class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 hover:bg-white/10 transition-all">
                    ...
                    ($product->category === 'ACCESSORY' ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400') }}">
                                    ...
                                    @if($product->network_type)
                                        <span
                                            class="px-1.5 py-0.5 text-xs rounded bg-slate-700 text-slate-300">{{ $product->network_type }}</span>
                                    @endif
                            ...
                            <div class="text-right flex-shrink-0">
                                <p class="text-lg font-bold text-emerald-400">₹{{ number_format($product->selling_price, 0) }}
                                </p>
                    ...
                    ($product->is_low_stock ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400') }}">
                            ...
                            <div class="flex flex-col gap-1">
                                <button wire:click="edit({{ $product->id }})"
                                    class="p-2 text-slate-400 hover:text-violet-400 transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    @if($showModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data
            x-init="document.body.style.overflow = 'hidden'" x-on:remove="document.body.style.overflow = ''">
                ...
                <form wire:submit="save" class="p-6 space-y-5">
                    ...
                    @if($category === 'MOBILE')
                        <div>
                            ...
                            <select wire:model="brand"
                                class="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-violet-500">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    @foreach($networkTypes as $type)
                                        <label class="relative cursor-pointer">
                                            <input wire:model="network_type" type="radio" name="network_type" value="{{ $type }}"
                                                class="sr-only peer">
                                            <div
                    ...
                    <button type="submit"
                        ...
                        <span wire:loading.remove
                            wire:target="save">{{ $editingId ? 'Update Product' : 'Add Product' }}</span>
